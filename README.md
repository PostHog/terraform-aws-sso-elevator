...
| <a name="input_config"></a> [config](#input\_config) | SSO Elevator config. Each item specifies access rules distinguished by ResourceType: 'Account' (default) for AWS account access with PermissionSet, or 'Group' for SSO group membership. | `any` | `[]` | no |
