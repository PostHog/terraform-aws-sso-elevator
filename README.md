...
| <a name="input_secrets_ui_url"></a> [secrets\_ui\_url](#input\_secrets\_ui\_url) | Optional URL to a self-service secrets UI. When set, a reply is posted in the request thread whenever someone requests the `secrets-editor` permission set, pointing them at the UI so they can skip the AWS grant for routine secret edits. Leave blank to disable the reply. | `string` | `""` | no |
