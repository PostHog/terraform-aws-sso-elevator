...
|------|-------------|------|---------|:--------:|
| <a name="input_access_docs_url"></a> [access\_docs\_url](#input\_access\_docs\_url) | Optional URL to your access documentation. When set, a 'See the access docs' link is shown beside the permission-set/group dropdown in the request modals to help requesters pick the right one. Leave blank to hide the link. | `string` | `""` | no |
| <a name="input_account_sections"></a> [account\_sections](#input\_account\_sections) | Optional, ordered list of sections used to group the "AWS Account(s)" multi-select<br/>in the Slack request modal. Each section is { name = "<label>", accounts = ["<id>", ...] }.<br/>Sections render in the order listed; accounts are sorted alphabetically by name within<br/>each section. An account listed in multiple sections appears only in the first. Accounts<br/>not listed in any section fall into a trailing "Other" section. Listing an account id the<br/>requester is not eligible for is harmless (it is simply not shown).<br/>Example:<br/>[<br/>  { name = "Production",  accounts = ["111111111111", "222222222222"] },<br/>  { name = "Dev / Infra", accounts = ["333333333333"] },<br/>]<br/>Leave empty ([]) to keep a single flat, alphabetically-sorted list.<br/>Note: section labels are truncated to 75 characters, and the account list is capped at 99 entries<br/>(Slack's per-select option limit) the same as the flat list -- sections regroup accounts, they do<br/>not raise the cap. | `list(object({ name = string, accounts = list(string) }))` | `[]` | no |
...
| <a name="input_cleanup_alert_slack_group"></a> [cleanup\_alert\_slack\_group](#input\_cleanup\_alert\_slack\_group) | Slack usergroup ID (e.g. `S12345`, same format as `approver_groups` entries) to ping on rollback-failure alerts so an on-call admin is paged when SSO Elevator cannot auto-roll-back a grant whose scheduled revoke failed to create. Leave blank to post without a ping. | `string` | `""` | no |
...
| <a name="input_event_bus_arn"></a> [event\_bus\_arn](#input\_event\_bus\_arn) | EventBridge bus ARN to publish access change events to. If empty, no events are published. | `string` | `""` | no |
...
| <a name="input_permission_set_hints"></a> [permission\_set\_hints](#input\_permission\_set\_hints) | Optional mapping of permission set names (or ARNs) to a hint shown when that permission set<br/>is selected. Keys are AWS SSO permission set names or ARNs (as used in config statements);<br/>a name match wins over an ARN match. Values are Slack mrkdwn, rendered verbatim, so links use<br/>Slack's <URL\|label> syntax.<br/>Example:<br/>{<br/>  "secrets-editor" = ":bulb: Most secret changes don't need this role -- read and edit directly in <https://secrets.example.com\|the secrets UI>."<br/>  "prod-admin"     = ":warning: Read <https://wiki.example.com/prod-admin\|the runbook> before requesting."<br/>}<br/>The hint appears twice: as a grey context line under the permission set dropdown the moment the<br/>role is selected, and as a reply in the request's Slack thread after submission. It is purely<br/>informational -- it never blocks or gates a request.<br/>Permission sets without a mapping show no hint. This is independent of "access\_docs\_url": when<br/>both are set, the general docs link and the role-specific hint are shown together.<br/>Leave empty ({}) to disable.<br/>Note: hints are truncated to 3000 characters (Slack's context block limit). | `map(string)` | `{}` | no |
...
| <a name="input_slack_error_channel_id"></a> [slack\_error\_channel\_id](#input\_slack\_error\_channel\_id) | Slack channel ID for operator/system error notifications (unexpected exceptions, rollback-failure alerts, attribute-sync errors). Leave blank to fall back to slack\_channel\_id. | `string` | `""` | no |
